	...
	def get_recommended_items(self, settings):
		...
		if settings.show_price:
			...
			for item in items:
				...
				# Override with PCK price if available — PCK is the default display UOM
				pck_price = frappe.db.get_value(
					"Item Price",
					{"item_code": item.item_code, "price_list": selling_price_list, "uom": "PCK", "selling": 1},
					["price_list_rate", "currency"],
					as_dict=True,
				)
				if pck_price and item.price_info:
					fmt = frappe.utils.fmt_money(pck_price.price_list_rate, currency=pck_price.currency)
					item.price_info["price_list_rate"] = pck_price.price_list_rate
					item.price_info["formatted_price"] = fmt
					item.price_info["formatted_price_sales_uom"] = fmt
